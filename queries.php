...
$queries[2] = array(
							...
							'query' => 
'let 
  $referenceWay1 := xosm_rtj:getElementByName(. ,"Calle Calzada de Castro"),
  $referenceWay2 := xosm_rtj:getElementByName(. ,"Avenida Nuestra Señora de Montserrat"),
  $referenceLayer := xosm_rtj:getLayerByName(. ,"Calle Calzada de Castro",0),
  $onewaysCrossing := fn:filter($referenceLayer,xosm_sp:isCrossing(?, $referenceWay1))
return
...
$queries[3] = array(
							...
							'query' => 
'let $referenceWay := xosm_rtj:getElementByName(. , "Calle Calzada de Castro"),
  $referenceLayer := xosm_rtj:getLayerByName(. , "Calle Calzada de Castro",0.001),
  $onewaysAllEndingTo := fn:filter($referenceLayer, xosm_sp:isEndingTo(?, $referenceWay))
return
fn:filter(fn:for-each($onewaysAllEndingTo, xosm_rtj:getLayerByElement(. ,?,0.001)),
...
return
fn:filter(fn:for-each($crossing, xosm_rtj:getLayerByElement(. , ? , 0.001)),
...
return fn:filter(xosm_rtj:getLayerByElement(. ,$pharmacies,0.001), xosm_kw:searchKeywordSet(?,("highway" , "pharmacy")))',
...
return fn:filter($layer , xosm_kw:searchKeywordSet(? , ("hotel" , "bar" , "restaurant")))',
